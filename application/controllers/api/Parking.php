...
class Parking extends MY_RestController {
    ...
    public function car_parkings_get() {
        ...
        if($car_parkings) {
            foreach($car_parkings as $key => $val) {
                //Append ebedded images to the array
                $car_parkings[$key]['images'] = $this->image->get_parking_image_urls($car_parkings[$key]['parking_id']);
            }
    ...
    public function wheelchair_parkings_get() {
        ...
        if($wheelchair_parkings) {
            foreach($wheelchair_parkings as $key => $val) {
                //Append ebedded images to the array
                $wheelchair_parkings[$key]['images'] = $this->image->get_parking_image_urls($wheelchair_parkings[$key]['parking_id']);
            }
